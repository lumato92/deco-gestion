...
function FormProducto({ id }: { id: number | null }) {
        ...
        if (prodRes.data) {
          ...
          if (p.categoria_id) {
            const subRes = await supabase
              .from('subcategorias').select('id, nombre')
              .eq('categoria_id', p.categoria_id).order('orden')
            setSubcategorias(subRes.data ?? [])
          }
        }
        setReceta((recetaRes.data ?? []).map((r: any) => ({
          insumo_id: r.insumo_id, nombre: r.insumo_nombre,
          unidad: r.unidad, cantidad: r.cantidad, costo_item: r.costo_item,
        })))
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    ...
    createClient()
      .from('subcategorias').select('id, nombre')
      .eq('categoria_id', categoriaId).order('orden')
  ...
  useEffect(() => {
    ...
    setResultadosInsumo(
      todosInsumos.filter(i => normalizar(i.nombre).includes(q)).slice(0, 8)
    )
  ...
  const handleGuardar = async () => {
    ...
    try {
      const datos = {
        ...
        stock, precio, minimo, publicado, destacado, estado,
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs text-gray-400 mb-1">
            Catálogo › Productos › {esNuevo ? 'Nuevo' : 'Editar'}
          </p>
      ...
      <div className="grid grid-cols-[1fr_280px] gap-4 items-start">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] text-gray-500 uppercase tracking-wide">Precio de venta</label>
